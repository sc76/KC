...
//		maxNumberOfSpecialUnitType4 = 2; // 퀸
	...
	public void getTargetPositionForDefence(){
		...
		if(existHatcheryInCenterRegion == true){
			DEFENCE_POSITION = CENTER_POSITION;
			DEFENCE_TILEPOSITION = CENTER_TILEPOSITION;
		}
		// center를 못 먹었으면
...
//				}
	...
	Position getCalcuatePosition(Unit unit){
		...
		// 12시
		if(unit.getID() % 4 == 0){
			Position p = new Position(unit.getPosition().getX(), unit.getPosition().getY() - Config.TILE_SIZE*5);
			
			List<Unit> radiusUnits = MyBotModule.Broodwar.getUnitsInRadius(p, Config.TILE_SIZE*2);
			for(Unit aroundUnit : radiusUnits){
				if(aroundUnit.getPlayer() == enemyPlayer){
					p = new Position(unit.getPosition().getX(), unit.getPosition().getY() + Config.TILE_SIZE*5); 
					break;
				}
			}
			return p;
		...
		// 3시
			...
			Position p = new Position(unit.getPosition().getX() + Config.TILE_SIZE*5, unit.getPosition().getY());
			
			List<Unit> radiusUnits = MyBotModule.Broodwar.getUnitsInRadius(p, Config.TILE_SIZE*2);
			for(Unit aroundUnit : radiusUnits){
				if(aroundUnit.getPlayer() == enemyPlayer){
					p = new Position(unit.getPosition().getX() - Config.TILE_SIZE*5, unit.getPosition().getY());  
					break;
				}
			}
	...
	public void update() {
		...
		executeAdvancedDefenceBuildingCombat();
		...
		for(Unit unit : myPlayer.getUnits()){
			...
			if(unit.getType() == UnitType.Zerg_Creep_Colony){
				...
				if(existUnitTypeInRegion(enemyPlayer, (UnitType)null, creepRegion, false, false)){
					...
					System.out.println("find enemyPlayer !! ");
					System.out.println("find enemyPlayer !! ");
					System.out.println("find enemyPlayer !! ");
					System.out.println();
	...
	boolean isTimeToStartDefense() {
		...
		if(Race.Terran == enemyRace){
			...
			if(isInitialBuildOrderFinished == false){
				...
				// 저글링, 히드라, 럴커
				else if (myPlayer.hasResearched(TechType.Lurker_Aspect) == true
						&& myCombatUnitType2List.size() < necessaryNumberOfDefenceUnitType2   // 히드라
						&& myCombatUnitType3List.size() <= 1   // 럴커
						){
					countDefenceMode++;
		...
		for (Unit worker : WorkerManager.Instance().getWorkerData().getWorkers()) {
			...
			while(iter.hasNext()){
				...
				if(commandUtil.IsValidEnemyGroundAttackUnit(unit)){
					...
					if(unit.getType().canAttack() == false) continue; // 공격력이 없으면, 메딕.. 
					...
					if(worker.getDistance(enemyUnit) < distanceWorkerAround){
						...
						if(enemyUnit.getType().isWorker() == true || enemyUnit.isStartingAttack() || enemyUnit.isAttacking() || worker.isUnderAttack()){
							
							countWorkersToCanAttak = 1;
							...
							if(WorkerManager.Instance().getWorkerData().getWorkerJob(worker) == WorkerData.WorkerJob.Minerals
								...
								remainHitPoint = (worker.getHitPoints()*1.0)/(worker.getInitialHitPoints()*1.0);
...
//				if(DEBUG) System.out.println();
			...
			// sc76.choi 스톰을 맡고 있지 않으면
				...
				// sc76.choi 적이 없으면 모이는데, 확장이 시작되었으면 태어난 자리에 있게 한다.
					...
					if(unit.getDistance(myMainBaseLocation.getPosition()) > Config.TILE_SIZE*30){
						commandUtil.move(unit, DEFENCE_POSITION);
					}else{
						if(unit.isIdle()){
							commandUtil.move(unit, DEFENCE_POSITION);
...
//			System.out.println();
	...
	private Unit findAttackTargetForExpansionDefence() {
	    ...
	    for (Unit unit : MyBotModule.Broodwar.enemy().getUnits()) {
	    	...
	    	if(unit.getDistance(myFirstExpansionLocation) <= Config.TILE_SIZE*11){
...
//    	if(DEBUG) System.out.println("isTimeToBuildState end");
//    	if(DEBUG) System.out.println();
    }
	...
	void updateVariables(){
		...
		while (it.hasNext()) {
			...
			if (unit.getType() == myCombatUnitType1) {
				...
				if(isInitialBuildOrderFinished == true && enemyRace == Race.Zerg){
			...
			// 히드라
				...
				if(commandUtil.IsValidUnit(unit) == false) return;
	...
	public void updatebuildOrderArrayForProtoss(){
		...
		// sc76.choi 그레이트 스파이어만, 올라가 있을 때,
				...
				if(selfGas < 200 && myPlayer.completedUnitCount(UnitType.Zerg_Mutalisk) >= 2){
					...
					buildOrderArrayOfMyCombatUnitType = new int[]{1, 1, 2, 1, 2, 1, 1, 2, 2, 2, 1, 4}; 	// 저글링 히드라 히드라 럴커 뮤탈 뮤탈
				}else{
	...
	public void updatebuildOrderArrayForTerran(){
		...
		if(myPlayer.completedUnitCount(UnitType.Zerg_Hydralisk_Den) > 0 
			...
			if(selfGas < 200){
				...
				buildOrderArrayOfMyCombatUnitType = new int[]{3, 1, 2, 2, 3, 1, 2, 1, 3, 2, 2, 3}; 	// 저글링 히드라 히드라 럴커 뮤탈 뮤탈
			}
			...
			if(selfGas < 200 && myPlayer.completedUnitCount(UnitType.Zerg_Hydralisk) <= 0){
				...
				if(enemyRace == Race.Terran){
					if(selfGas < 200){
						...
						buildOrderArrayOfMyCombatUnitType = new int[]{3, 1, 2, 2, 3, 4, 1, 3, 2, 3, 4, 4}; 	// 저글링 히드라 히드라 럴커 뮤탈 뮤탈
					}
				}else{
					seqBuildOrderStep = 60;
...
//		}
		...
		int gasWorkers = WorkerManager.Instance().getNumWorkers();
...
//				&& ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Zerg_Creep_Colony, null) == 0
				...
				// 확장에 low로 추가				
				BuildManager.Instance().buildQueue.queueAsLowestPriority(myDefenseBuildingType1, 
						myFirstExpansionLocation.getTilePosition(), true);
...
//				&& ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Zerg_Sunken_Colony, null) == 0
				...
				// 확장에 low로 추가
				BuildManager.Instance().buildQueue.queueAsLowestPriority(myDefenseBuildingType2, 
						myFirstExpansionLocation.getTilePosition(), false);
...
//				}
				...
				if (allCountOfSpecialUnitType3 < maxNumberOfTrainSpecialUnitType3) {
					// sc76.choi 뮤탈이 4마리 이상 있으면, 가디언이 1마리 이상 있으면 생산시작한다.
					if(myCombatUnitType4List.size() >= 2
