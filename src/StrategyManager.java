	...
	public void controlAdvancedDefenceBuildingCombat() {
		...
		for(Unit unit : myPlayer.getUnits()){
			...
			if(unit.getType() == UnitType.Zerg_Creep_Colony){
				...
				if(existUnitTypeInRegion(enemyPlayer, (UnitType)null, creepRegion, false, false)){
					...
					if(BuildManager.Instance().buildQueue.getItemCount(UnitType.Zerg_Sunken_Colony) == 0
						...
						BuildManager.Instance().buildQueue.queueAsHighestPriority(UnitType.Zerg_Sunken_Colony, unit.getTilePosition(), true);
					}
...
//				urgent_AirDefenceBuilding1 = false;
//				urgent_AirDefenceBuilding2 = false;
//			}
//			else if(isInitialBuildOrderFinished == true
//						&& existUnitTypeInRegion(myPlayer, UnitType.Zerg_Creep_Colony, myMainBaseLocation.getRegion()) == true					
//						&& existUnitTypeInRegion(myPlayer, UnitType.Zerg_Spore_Colony, myMainBaseLocation.getRegion()) == false
	...
	public void executeMaintenance(){
		...
		if(isInitialBuildOrderFinished == true){
			...
			if(existHatcheryInMyFirstExpansion == false){
				...
				if(enemyUnitsInMyFirstExpansion > 0){
					...
					&& ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Zerg_Hydralisk_Den, null) == 0) 
			{
					...
					BuildManager.Instance().buildQueue.queueAsHighestPriority(UnitType.Zerg_Hydralisk_Den, true);
			}		
...
//				}
				...
				// blockDefence2Dragon8_P면 발업  후, 공격에 가담
				if(buildState == BuildState.blockDefence2Dragon8_P){
					if(myPlayer.getUpgradeLevel(UpgradeType.Muscular_Augments) == 0
				...
				// 저글링 + 히드라
				if((myCombatUnitType1List.size() + myCombatUnitType1ListAway.size()) >= necessaryNumberOfCombatUnitType1
					&& (myCombatUnitType2List.size() + myCombatUnitType2ListAway.size()) >= necessaryNumberOfCombatUnitType2){
					isNecessaryNumberOfCombatUnitType = true;
				}
				// 히드라				
				else if((myCombatUnitType2List.size() + myCombatUnitType2ListAway.size()) >= necessaryNumberOfCombatUnitType2){
					isNecessaryNumberOfCombatUnitType = true;
				}
				// 저글링 + 럴커				
				else if((myCombatUnitType1List.size() +  + myCombatUnitType1ListAway.size()) >= necessaryNumberOfDefenceUnitType1
						&& (myCombatUnitType3List.size() + myCombatUnitType3ListAway.size()) >= necessaryNumberOfCombatUnitType3){
					isNecessaryNumberOfCombatUnitType = true;
				}
...
//		int mapWidth = MyBotModule.Broodwar.mapWidth();		// 128
	...
	void commandMyWorkerToAttack(){
		...
		if(isInitialBuildOrderFinished == false && enemyUnitsInMyFirstExpansion >= 4 && myUnitsInMyFirstExpansion <= 2) {
...
//		System.out.println();
	...
	public Unit getEnemyWeakUnitsInRadius(List<Unit> units){
		...
		for(Unit enemyUnit : units){
			...
			if(enemyUnit.getPlayer() == enemyPlayer && enemyUnit.getType().canAttack() == true && spiderMine == null){
	...
	boolean controlCombatUnitType1(Unit unit) {
		...
		if(combatState == CombatState.attackStarted){
			...
			if(isInitialBuildOrderFinished == false && (enemyRace == Race.Terran || enemyRace == Race.Protoss)){
...
//					if(BWTA.getRegion(unit.getPosition()) == BWTA.getRegion(enemyMainBaseLocation.getPosition())){
			...
			// sc76.choi 스톰을 맡고 있지 않으면
				...
				// sc76.choi DEFENCE_POSITION에 적이 있으면 
					...
					List<Unit> unitsAttackingRadius = unit.getUnitsInRadius(Config.TILE_SIZE*6);
					boolean canAttackNow = KCSimulationManager.Instance().canAttackNow(unitsAttackingRadius);
...
//						Position fastMutalisk_Z_for_hydralisk = null;
//						
//						if(buildState == BuildState.fastMutalisk_Z){
//							
//						}
			...
			// 공격할때
				...
				// sc76.choi 스톰을 맡고 있지 않으면
					...
					KCSimulationResult canAttackNow = KCSimulationManager.Instance().canAttackNow2(myUnitsInRadius);
					
					if(canAttackNow.isCanAttackNow() && unit.getGroundWeaponCooldown() == 0){
						if(unit.isAttacking() == false){
							if(targetWeakUnit != null){
								commandUtil.attackUnit(unit, targetWeakUnit);
							}else{
		...
		if (combatState == CombatState.defenseMode) {
			...
			if (unit.isBurrowed() == false) {			
				...
				// sc76.choi 나의 본거지와 멀리 있으면 귀환
					...
					// TODO DEFENCE모드에도 멀티에서 오기 때문에, 적을 만나면 버로우 해야 한다.
//					commandUtil.move(unit, DEFENCE_POSITION);
			...
			// sc76.choi 방어나, 초기 모드 일때
				...
				for(Unit overload : OverloadManager.Instance().getOverloadData().getOverloads()){
					if(OverloadManager.Instance().getOverloadData().getJobCode(overload) == 'A'
						...
						if(overload.getDistance(DEFENCE_POSITION) > Config.TILE_SIZE*35){
				...
				if (unit.getEnergy() > 150 && myPlayer.hasResearched(TechType.Spawn_Broodlings)) {
					...
					if(unit.isIdle()){
						...
						if(enemyUnitForQueen == null){
							...
							if(Config.IS_DRAW) MyBotModule.Broodwar.drawLineMap(unit.getPosition(), enemyUnitForQueen.getTargetPosition(), Color.White);
						}
					}
				}
...
//							|| enemyPlayer.incompleteUnitCount(UnitType.Zerg_Lair) > 0
    	...
    	// 테란의 경우
	   		...
	   		if((countEnemyBasicCombatUnitType >= 5 && countEnemyBasicCombatBuildingType >= 2)
	   		...
	   		}else if (buildState == BuildState.hardCoreMarine_T
    				&& MyBotModule.Broodwar.getFrameCount() > (24 * 60 * 10)){
    			buildState = BuildState.normalMode;
    		}
	    	...
	    	else if (buildState == BuildState.fastVulture_T
	    				&& MyBotModule.Broodwar.getFrameCount() > (24 * 60 * 10)){
	    			buildState = BuildState.normalMode;
	    	}
	    	else if (countEnemyGoliathCombatUnitType >= 8
	    			&& countEnemyTankCombatUnitType >= 3){
    			buildState = BuildState.vulture_Galia_Tank_T;
	    	}
	   		
	   		if(countEnemyTankCombatUnitType >= 3){
...
//		}
	...
	private void drawStrategyManagerStatus() {
		...
		MyBotModule.Broodwar.drawTextScreen(410, 60, red + "Defence Pos. : " + DEFENCE_TILEPOSITION + DEFENCE_POSITION);
		
		//MyBotModule.Broodwar.drawTextScreen(440, 70, "isDefence : " + isNecessaryNumberOfDefencedUnitType());
		MyBotModule.Broodwar.drawTextScreen(410, 70, "myUnit : " + 
		"[" + (myCombatUnitType1List.size() + myCombatUnitType1ListAway.size()) + "/" + necessaryNumberOfCombatUnitType1 +"/" +  necessaryNumberOfDefenceUnitType1 +"]" +
		"[" + (myCombatUnitType2List.size() + myCombatUnitType2ListAway.size()) + "/" + necessaryNumberOfCombatUnitType2 +"/" +  necessaryNumberOfDefenceUnitType2 +"]" +
		"[" + (myCombatUnitType3List.size() + myCombatUnitType3ListAway.size()) + "/" + necessaryNumberOfCombatUnitType3 +"/" +  necessaryNumberOfDefenceUnitType3 +"]" +
		" Air : " + bTimeToAirDefence);
		//MyBotModule.Broodwar.drawTextScreen(410, 80, "Time Air Defence : " + bTimeToAirDefence);		
...
//					if(myCombatUnitType2MultiDefenceList.isEmpty() == false){
//						System.out.println("1 myCombatUnitType2MultiDefenceList : " + myCombatUnitType2MultiDefenceList.get(0).getID() + " " + myCombatUnitType2MultiDefenceList.size());
//						System.out.println();
//					}
...
//					if(myCombatUnitType2MultiDefenceList2.isEmpty() == false){
//						System.out.println("2 myCombatUnitType2MultiDefenceList2 : " + myCombatUnitType2MultiDefenceList2.get(0).getID() + " " + myCombatUnitType2MultiDefenceList2.get(0).getTilePosition() +  " " +  myCombatUnitType2MultiDefenceList2.size());
//						System.out.println();
//					}
			...
			if (unit.getType() == myCombatUnitType1) {
				...
				// 멀리 떨어진 곳에서 생산된 유닛
				if(unit.getDistance(myMainBaseLocation.getPosition()) > Config.TILE_SIZE*50
						&& combatState == CombatState.defenseMode){
	...
	public void updatebuildOrderArrayForProtoss(){
		...
		// sc76.choi 히드라(럴커), 스파이어만 올라가 있을 때,
			...
			if(selfGas < 200){
				...
				buildOrderArrayOfMyCombatUnitType = new int[]{3, 1, 1, 1, 2, 2, 1, 2, 1, 1, 1, 4}; 	// 저글링 히드라 히드라 럴커 뮤탈 뮤탈
			}else{
		...
		// sc76.choi 그레이트 스파이어만, 올라가 있을 때,
				...
				if(selfGas < 200){
					...
					buildOrderArrayOfMyCombatUnitType = new int[]{1, 2, 2, 4, 6, 1, 2, 4, 6, 3, 1, 2}; 	// 저글링 히드라 히드라 럴커 뮤탈 뮤탈
				}
	...
	public void updatebuildOrderArrayForTerran(){
		...
		if(myPlayer.completedUnitCount(UnitType.Zerg_Hydralisk_Den) > 0 
			...
			if(selfGas < 100){
				...
				if(existHydralisk) buildOrderArrayOfMyCombatUnitType = new int[]{3, 1, 2, 2, 2, 3, 1, 1, 2, 1, 1, 2}; 	
		...
		// sc76.choi 스파이어만 올라가 있을 때,
			...
			if(selfGas < 200){
				...
				buildOrderArrayOfMyCombatUnitType = new int[]{1, 1, 1, 1, 1, 1, 1, 2, 2, 2, 1, 4};
				
			}else if(selfGas < 200){
				strBuildOrderStep = "T 40";
				
				if(existHydralisk) buildOrderArrayOfMyCombatUnitType = new int[]{1, 1, 2, 1, 1, 3, 1, 1, 2, 3, 1, 1};
				else buildOrderArrayOfMyCombatUnitType = new int[]{1, 1, 2, 2, 2, 1, 1, 1, 2, 2, 1, 1};
					
			}else{
				strBuildOrderStep = "T 50";
				
				if(existHydralisk) buildOrderArrayOfMyCombatUnitType = new int[]{1, 2, 2, 2, 3, 2, 1, 2, 1, 3, 2, 4};
				else buildOrderArrayOfMyCombatUnitType = new int[]{2, 1, 2, 2, 2, 4, 2, 2, 2, 2, 2, 4};
			}
		}
		// sc76.choi 스파이어, 울트라 카벤 모두 올라가 있을 때,
				...
				if(selfGas < 200){
					...
					if(existHydralisk) buildOrderArrayOfMyCombatUnitType = new int[]{1, 1, 1, 2, 2, 5, 1, 1, 2, 3, 5, 4};
					else buildOrderArrayOfMyCombatUnitType = new int[]{1, 1, 4, 2, 2, 5, 1, 1, 2, 2, 5, 4};
				}
		...
		// sc76.choi 그레이트 스파이어, 울트라 카벤 모두 올라가 있을 때,
				...
				if(selfGas < 200){
					...
					if(existHydralisk) buildOrderArrayOfMyCombatUnitType = new int[]{1, 1, 1, 2, 2, 1, 1, 2, 3, 2, 1, 4};
					else buildOrderArrayOfMyCombatUnitType = new int[]{1, 1, 1, 2, 2, 1, 1, 2, 2, 2, 1, 4};
				}else{
					strBuildOrderStep = "T 100";
					
					if(existHydralisk) buildOrderArrayOfMyCombatUnitType = new int[]{1, 1, 2, 4, 4, 1, 1, 2, 6, 3, 5, 6};
					else buildOrderArrayOfMyCombatUnitType = new int[]{1, 1, 2, 4, 4, 3, 1, 2, 6, 2, 5, 6};
				}
		...
		// sc76.choi 그레이트 스파이어만, 올라가 있을 때,
				...
				if(selfGas < 200){
					...
					if(existHydralisk) buildOrderArrayOfMyCombatUnitType = new int[]{1, 1, 2, 4, 6, 1, 1, 4, 6, 3, 1, 2}; 	// 저글링 히드라 히드라 럴커 뮤탈 뮤탈
					else buildOrderArrayOfMyCombatUnitType = new int[]{1, 1, 2, 4, 6, 3, 1, 4, 6, 2, 1, 2}; 	// 저글링 히드라 히드라 럴커 뮤탈 뮤탈
				}
			...
			if(myPlayer.completedUnitCount(UnitType.Zerg_Hydralisk_Den) <= 0){
				...
				buildOrderArrayOfMyCombatUnitType = new int[]{2, 2, 1, 2, 2, 1, 2, 2, 2, 1, 2, 1}; 	// 저글링 저글링 히드라 히드라 히드라 러커
			}
	...
	public void updatebuildOrderArrayForZerg(){
		...
		// sc76.choi 그레이트 스파이어만, 올라가 있을 때,
			...
			if(myPlayer.completedUnitCount(UnitType.Zerg_Hydralisk_Den) <= 0){
				...
				if(buildState == BuildState.fastMutalisk_Z){
					buildOrderArrayOfMyCombatUnitType = new int[]{2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 1}; 	// 저글링 저글링 히드라 히드라 히드라 러커
				}else{
...
//	}
	...
	public void moveUnitToDefencePosition(Unit unit){
		...
		if(combatState == CombatState.defenseMode){
			MOVE_POSITION = DEFENCE_POSITION;
		}else{
		...
		if((myCombatUnitType1ListAway.size() + myCombatUnitType2ListAway.size() + myCombatUnitType3ListAway.size()) >= 10){
			for(Unit awayUnit : myCombatUnitType1ListAway){
				if(awayUnit.isIdle()){
					commandUtil.attackMove(awayUnit, MOVE_POSITION);
				}
			}
			
			for(Unit awayUnit : myCombatUnitType2ListAway){
				if(awayUnit.isIdle()){
					commandUtil.attackMove(awayUnit, MOVE_POSITION);
				}
			}
			
			for(Unit awayUnit : myCombatUnitType3ListAway){
				if(awayUnit.isIdle()){
					commandUtil.attackMove(awayUnit, MOVE_POSITION);
				}
			}
	...
	public void excuteConfigration(){
		...
		if(buildState == BuildState.blockDefence2Dragon8_P){
			...
			// 저글링
   			...
   			Config.necessaryNumberOfCombatUnitType2AgainstProtoss = 20;
		...
		if(InformationManager.Instance().getTotalHatcheryCount() >= 2){
		...
		if(myOccupiedBaseLocations >= 3){
			...
			if(WorkerManager.Instance().getNumWorkers() >= 20){
				WorkerManager.Instance().getWorkerData().mineralAndMineralWorkerRatio = 1.5;
			}else{
					...
					if (unit.getType() == UnitType.Protoss_Nexus || unit.getType() == UnitType.Terran_Command_Center || unit.getType() == UnitType.Zerg_Larva) {
						if (unit.isTraining() == false && unit.isMorphing() == false) {
							...
							if (BuildManager.Instance().buildQueue
								...
								if(WorkerManager.Instance().getWorkerData().getNumWorkers() < 24){
									if(enemyRace == Race.Terran){
										if(myPlayer.completedUnitCount(UnitType.Zerg_Lair) > 0){
...
//		System.out.println("MyBotModule.Broodwar.self().supplyTotal() : " +  MyBotModule.Broodwar.self().supplyTotal());
		...
		if(isInitialBuildOrderFinished == false && InformationManager.Instance().getTotalHatcheryCount() <= 1){ 
//			if (){
...
//							base.getTilePosition(), false);
	...
	void excuteUrgenturgent_Add_Zergling1(){
		...
		if(urgent_Add_Zergling1 == false){
			BuildManager.Instance().buildQueue.queueAsHighestPriority(UnitType.Zerg_Zergling, true);
			BuildManager.Instance().buildQueue.queueAsHighestPriority(UnitType.Zerg_Zergling, true);
			BuildManager.Instance().buildQueue.queueAsHighestPriority(UnitType.Zerg_Zergling, true);
...
//							&& ConstructionManager.Instance().getConstructionQueueItemCount(InformationManager.Instance().getBasicCombatBuildingType(), null) == 0){
	...
	public BaseLocation getBestMultiLocation(){
		...
		if(numberOfMyCombatUnitTrainingBuilding >= countForMultibyRace && numberOfMyCombatUnitTrainingBuilding < 4){
